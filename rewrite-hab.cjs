...
import { BedDouble, ShoppingCart, ArrowLeft, X, MessageCircle, Phone } from "lucide-react";
import "../styles.css";
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: "40px" }}>
          {rooms.map((room) => (
            <motion.article 
              ...
              viewport={{ once: true }}
              style={{ backgroundColor: "var(--card)", borderRadius: "16px", overflow: "hidden", boxShadow: "0 10px 30px rgba(0,0,0,0.05)", border: "1px solid var(--border)", display: "flex", flexDirection: "column" }}
            >
              <div style={{ position: "relative", paddingTop: "66%" }}>
                <img src={room.images[0]} alt={room.name} style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", objectFit: "cover" }} />
                <div style={{ position: "absolute", top: "15px", right: "15px", backgroundColor: "white", color: "var(--ink)", padding: "5px 12px", borderRadius: "20px", fontWeight: "bold", fontSize: "0.9rem", boxShadow: "0 4px 10px rgba(0,0,0,0.1)" }}>
                  {room.guests} Huéspedes
                </div>
              </div>
              <div style={{ padding: "25px", flex: 1, display: "flex", flexDirection: "column" }}>
                <h3 style={{ fontFamily: "var(--font-display)", fontSize: "1.5rem", margin: "0 0 10px" }}>{room.name}</h3>
                <p style={{ opacity: 0.7, margin: "0 0 20px", fontSize: "0.95rem", lineHeight: "1.5", flex: 1 }}>{room.description}</p>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: "20px", borderTop: "1px solid var(--border)" }}>
                  <div>
                    <div style={{ fontSize: "0.8rem", textTransform: "uppercase", fontWeight: 700, color: "var(--muted-foreground)" }}>Por noche</div>
                    <div style={{ fontSize: "1.4rem", fontWeight: "bold", color: "var(--ink)" }}>\${room.price}</div>
                  </div>
                  <button 
                    onClick={() => addToCart(room)}
                    className="clay-button"
                    style={{ minHeight: "44px", height: "44px", padding: "0 20px" }}
                  >
                    Añadir
                  </button>
                </div>
